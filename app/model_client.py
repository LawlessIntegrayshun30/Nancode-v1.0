...
class ModelClient:
    def __init__(self, base_url: str, timeout: float = 30.0, client: Optional[httpx.AsyncClient] = None) -> None:
        """
    ...
    async def generate(self, prompt: str, **kwargs: Any) -> Dict[str, Any]:
        ...
        if self._client is None:
            async with httpx.AsyncClient(base_url=self.base_url) as client:
                response = await client.post("/generate", json=payload, timeout=self.timeout)
        ...
        response = await self._client.post(f"{self.base_url}/generate", json=payload, timeout=self.timeout)
        response.raise_for_status()
        return response.json()
